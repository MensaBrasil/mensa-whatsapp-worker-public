...
async function triggerTwilioOrRemove(phoneNumber, reason) {
  try {
    ...
    if (timeElapsed > oneWeek) {
      ...
      console.log(
        `Twilio Flow retriggered for ${phoneNumber} with the same reason after one week: ${reason}, Execution SID: ${execution.sid}`,
      );
    ...
    if (timeElapsed > waitingPeriod) {
      console.log(
        `Waiting period ended for ${phoneNumber} with reason: ${reason}. User should be removed.`,
      );
      return true; // Waiting period has ended, user should be removed
    }

    console.log(
      `Skipped Twilio Flow for ${phoneNumber}. Waiting period has not yet expired for reason: ${reason}`,
    );
